package com.example.crypx.presentation.coin_detail
...
@Composable
fun SwipeableCards(coins: List<Coin>) {
    var coinList = coins.take(5).reversed()

    Log.d("SwipeableCards", "SwipeableCards: ${coins.size}")
    Log.d("SwipeableCards", "SwipeableCards: ${coinList.size}")
    ...
    Box(
        ...
        coinList.forEachIndexed { idx, coin ->
            key(coin) {
                val currentColor = colorList[idx];
                Log.d("SwipeableCards", "SwipeableCardsTOp: ${coinList}")
                val currentCoin = coinList[idx]
                SwipeableCard(
                    order = idx,
                    coin = currentCoin,
                    totalCount = coinList.size,
                    backgroundColor = currentColor,
                    onMoveToBack = {
                        Log.d("SwipeableCards", "SwipeableCardsBefore: ${colorList}")
                        colorList = listOf (colorList[coinList.size-1]) + (colorList-(colorList[coinList.size-1]))
                        Log.d("SwipeableCards", "SwipeableCardsAfter: ${colorList}")
                        Log.d("SwipeableCards", "SwipeableCardsBefore: ${coinList}")
                        coinList = listOf(coin) + (coinList - coin)
                        Log.d("SwipeableCards", "SwipeableCardsAfter: ${coinList}")
                    })
            }
        }
    }
...
fun SwipeableCard(
    ...
    val animatedScale by animateFloatAsState(
        targetValue = 1f - (totalCount - order) * 0.09f,
    )
    val animatedYOffset by animateDpAsState(
        targetValue = ((totalCount - order) * -1).dp,
    )
    Box(
        modifier = Modifier
        ...
        SampleCard(coin= coin,backgroundColor = backgroundColor)
    }
}
@Composable
fun SampleCard(coin: Coin, backgroundColor: Color = Color.White) {
    Card(
        modifier = Modifier
            .height(400.dp)
            .offset(y = (260).dp),
                        ...
                        launch {
                            offsetY.animateTo(targetValue = 0f,
                                ...
                                if (value <= -size.height * 2 && !clearedHurdle) {
                                    onMoveToBack()
                                    Log.d("TAG", "swipeToBack: implemented")
                                    clearedHurdle = true
                                }
                            }
                        }
